...
// Strategy layer
let friendlySkies, ufoAnalysis, sequentialInterceptions, gameDate;

// Ufo
let alienResearch, alwaysHit;

alienResearch = document.getElementById("alien_research");

// Game date to be implemented at a later date
...
function simulation() {
	...
	statsGreen = calculateInterceptorStats(
		interceptorGreen,
		ufo,
		interceptionGreen
	);
	...
	resultsBlue = calculateResults(
		interceptionBlue.outcome,
		interceptorBlue.health,
		ufo.health
	);
	resultsGreen = calculateResults(
		interceptionGreen.outcome,
		interceptorGreen.health,
		ufo.health
	);
	resultsRed = calculateResults(
		interceptionRed.outcome,
		interceptorRed.health,
		ufo.health
	);
	...
	if (sequentialInterceptions) {
		...
		columnResultRed[0].innerHTML =
			resultsRed[0] + resultsGreen[0] + resultsBlue[0] + "%";
	}
}
	...
	for (let i = 0; i < timers.length; i++) {
		...
		if (
			...
			return updateKillAbort(
				outcome,
				interception.xcomHealth,
				interception.ufoHealth
			);
...
// Functions to update and display air combat results
function calculateResults(outcome, interceptorHealth, ufoHealth) {
	...
	results.push(
		Math.round((kill.xcomHealth / kill.number / interceptorHealth) * 100)
	);
	//  Kill Abort calcs
	results.push(Math.round((killAbort.number / simulations) * 100));
	results.push(
		Math.round((killAbort.ufoHealth / killAbort.number / ufoHealth) * 100)
	);
	results.push(
		Math.round(
			(killAbort.xcomHealth / killAbort.number / interceptorHealth) * 100
		)
	);
	//  Stalemate calcs
	results.push(Math.round((stalemate.number / simulations) * 100));
	results.push(
		Math.round((stalemate.ufoHealth / stalemate.number / ufoHealth) * 100)
	);
	results.push(
		Math.round(
			(stalemate.xcomHealth / stalemate.number / interceptorHealth) * 100
		)
	);
	//  Death Abort calcs
	results.push(Math.round((deathAbort.number / simulations) * 100));
	results.push(
		Math.round((deathAbort.ufoHealth / deathAbort.number / ufoHealth) * 100)
	);
...
// Functions to calculate and display interceptor stats
function calculateInterceptorStats(interceptor, ufo, interception) {
	let stats = [];

	let armorMitigation = interception.armorMitigation(
		ufo.armor,
		interceptor.penetration
	);
	let minDmgPerHit = interceptor.baseDmg * (1 - armorMitigation);
	let maxDmgPerCrit = minDmgPerHit * 3;
	let primaryDPS = damagePerSecond(
		minDmgPerHit,
		interception.xcomCritChance,
		interception.xcomHitChance,
		interceptor.shotDelay
	);
	let secDmgPerHit = interceptor.secondaryDmg * (1 - armorMitigation);
	let secondaryDPS = interceptor.secondary
		? damagePerSecond(
				secDmgPerHit,
				interception.xcomSecondaryCritChance,
				interception.xcomSecondaryHitChance,
				interceptor.secondaryShotDelay
		  )
		: 0;
